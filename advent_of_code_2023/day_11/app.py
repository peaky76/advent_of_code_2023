...
def get_galaxies(lines):
    return [(x, y) for x, row in enumerate(lines) for y, col in enumerate(row) if col == "#"]
...
def sum_shortest_paths(file, *, expansion_size=2):
    ...
    for galaxy_1, galaxy_2 in galaxy_pairs:
        ...
        horizontal_expansion = sum(x in range(min_x, max_x) for x in empty_rows) * (expansion_size - 1)
        vertical_expansion = sum(y in range(min_y, max_y) for y in empty_cols) * (expansion_size - 1)
        shortest_distance = manhattan_distance(galaxy_1, galaxy_2) + horizontal_expansion + vertical_expansion
