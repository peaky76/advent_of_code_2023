...
def follow_multi_path(lr_instructions, wasteland_map):
    ...
    moves_dictionary = {
        key: {key: [] for key in destination_locations} for key in start_locations
    }
    ...
    loops = [
        moves[0]
        for val in moves_dictionary.values()
        for moves in val.values()
        if len(moves) == 2
    ]
...
def get_step_count(file, *, use_ghosts=False):
    ...
    wasteland_map = {
        k: v for line in lines[2:] for k, v in map_line_to_dict(line).items()
    }
