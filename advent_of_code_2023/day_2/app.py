...
def sum_of_possible_games(lines, limit_rgb=(12, 13, 14)):
    return sum(game["id"] for game in get_games(lines) if is_game_possible(game["rgb"], limit_rgb))
...
# # PART TWO
example_answer = sum(power(min_possible_balls(game["rgb"])) for game in get_games(read_input("./day_2/example_input")))
print(f"Example answer: {example_answer}")
puzzle_answer = sum(power(min_possible_balls(game["rgb"])) for game in get_games(read_input("./day_2/puzzle_input")))
print(f"Puzzle answer: {puzzle_answer}")
