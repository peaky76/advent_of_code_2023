...
def has_adjacent_indicator(part_loc_and_len, part_indicators):
    ...
    if any(
        is_adjacent(part_bit, part_indicator)
        for part_bit in coords
...
def sum_gear_ratios(file):
    ...
    for gear in gears:
        adjacent_parts = [
            ...
            if has_adjacent_indicator(
                (part_number[1], part_number[2], len(str(part_number[0]))), [gear]
            )
        ]
...
def find_valid_part_numbers(file):
    ...
    return [
        ...
        for part_number in part_numbers
        if has_adjacent_indicator(
            (part_number[1], part_number[2], len(str(part_number[0]))), part_indicators
        )
    ]
